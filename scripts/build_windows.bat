...
set MY_GLOG_ROOT=%LIBS_ROOT%\glog-0.6.0
set GLOG_INCLUDE_DIR=%MY_GLOG_ROOT%\include
set GLOG_LIBRARY=%MY_GLOG_ROOT%\lib\glogd.lib
...
cmake -DGLEW_ROOT=%MY_GLEW_ROOT% -DGLEW_USE_STATIC_LIBS=1 -DGLEW_VERBOSE=%GLEW_VERBOSE% -DGLEW_INCLUDE_DIR=%GLEW_INCLUDE_DIR% -DGLEW_STATIC_LIBRARY_RELEASE=%GLEW_STATIC_LIBRARY% -DGLFW_ROOT=%MY_GLFW_ROOT% -DGLFW_INCLUDE_DIR=%GLFW_INCLUDE_DIR% -DGLFW_LIBRARY=%GLFW_LIBRARY% -DGLM_INCLUDE_DIR=%GLM_INCLUDE_DIR% -DGLOG_ROOT=%MY_GLOG_ROOT% -DGLOG_INCLUDE_DIR=%GLOG_INCLUDE_DIR% -DGLOG_LIBRARY=%GLOG_LIBRARY% -DZLIB_INCLUDE_DIR=%ZLIB_INCLUDE_DIR% -DZLIB_LIBRARY=%ZLIB_LIBRARY% -DPNG_PNG_INCLUDE_DIR=%PNG_INCLUDE_DIR% -DPNG_LIBRARY=%PNG_LIBRARY% -B ./build -S .
